...
class InputRelated {
  ...
  init(parent, onSelect=null) {
    ...
    this.elmArrow.onclick=(e)=> HClass.find(e.target, 'down')?this.openList({target:this.elmName}):this.closeList();
//     this.elmArrow.onclick=(e)=> alert(this.elmName.className);
  ...
  onBlur(e) {
    setTimeout(()=>HClass.find(this.elmArrow, 'down') && (e.target.value!==e.target._value || this.elmId.value!==this.elmId._value)?this.fillChoice():null, 300);
  }

  openList(e) {
    HClass.replace(this.elmArrow, 'down', 'up');
  ...
  closeList() {
    ...
    HClass.replace(this.elmArrow, 'up', 'down');
  }

}

class FundRelated extends InputRelated {
  constructor() {
    super();
    this.codeHTML=`<!--
...
class GenreRelated extends InputRelated {
  constructor() {
    super(parent);
    this.codeHTML=`<!--
