...
class FormTrans extends Form {
  constructor(parent) {
    super(parent);
    this.dateCard=new InputDate();
    this.incomeCard=new InputFloat();
    this.outgoCard=new InputFloat();
    this.genreCard=new GenreRelated();
    this.fundCard=new FundRelated();
    this.cardHTML=`
      <div class="record">
        <div class="left">
    ...
    `;
    this.dateGrid=Array(this.gridLength).fill().map(x=> x=new InputDate());
    this.incomeGrid=Array(this.gridLength).fill().map(x=> x=new InputFloat());
    this.outgoGrid=Array(this.gridLength).fill().map(x=> x=new InputFloat());
    this.genreGrid=Array(this.gridLength).fill().map(x=> x=new GenreRelated());
    this.fundGrid=Array(this.gridLength).fill().map(x=> x=new FundRelated());
    this.gridRowHTML=`
      <div class="record">
